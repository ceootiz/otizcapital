...
import { ArrowDownLeft, ArrowLeft, ArrowRight, ArrowUpRight, BriefcaseBusiness, CircleDollarSign, Gift, MessageCircleQuestion, RefreshCw } from "lucide-react";
...
export function InvestorMoneyMovementPage({ locale, ledger, totals, filters, statementsEnabled, performanceEnabled, contextualSupportEnabled, performanceEntries }: { locale: Locale; ledger: InvestorLedgerPage; totals: { profit: number; payout: number; reinvested: number }; filters: { type: string; from: string; to: string }; statementsEnabled: boolean; performanceEnabled: boolean; contextualSupportEnabled: boolean; performanceEntries: InvestorLedgerEntry[] }) {
  const t = COPY[locale];
    ...
    <Card><CardContent className="space-y-5 p-5 sm:p-6">
      ...
      <div className="space-y-3">
        {ledger.entries.length === 0 ? (hasFilters ? <p className="rounded-2xl bg-muted/30 p-8 text-center text-sm text-muted-foreground dark:bg-black/20">{t.emptyFiltered}</p> : <div className="rounded-2xl bg-muted/30 p-8 text-center dark:bg-black/20"><p className="font-semibold text-foreground">{t.emptyTitle}</p><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">{t.emptyDesc}</p><Link href={`/${locale}/investor/deposit`} className="mt-4 inline-flex rounded-full bg-foreground px-4 py-2 text-sm font-semibold text-background">{t.depositAction}</Link></div>) : ledger.entries.map((entry) => {
          ...
          const entryDate = new Intl.DateTimeFormat(locale === "zh" ? "zh-CN" : locale, { dateStyle: "medium" }).format(new Date(entry.occurredAt));
          const disputeContext = `${t.disputeContext}: ${t.types[entry.type]}, ${entryDate}, ${money(entry.amount, entry.currency, locale)}. ID: ${entry.id}`;
          ...
          return <div key={entry.id} className="flex flex-col gap-4 rounded-2xl border border-border p-4 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-start gap-3"><span className="rounded-xl bg-muted p-2.5 dark:bg-white/10"><Icon className="size-5" /></span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-foreground">{t.types[entry.type]}</p>{status ? <Badge>{status}</Badge> : null}<Badge variant="secondary">{entryDate}</Badge></div><p className="mt-1 break-words text-sm text-muted-foreground">{entry.detail ?? t.types[entry.type]}</p><div className="mt-2 flex flex-wrap gap-3"><Link href={`/${locale}${entry.href}`} className="inline-flex text-xs font-semibold text-amber-700 hover:underline dark:text-gold-100">{t.source}</Link>{contextualSupportEnabled ? <Link href={`/${locale}/investor/support?context=${encodeURIComponent(disputeContext)}#new-request`} className="inline-flex items-center gap-1 text-xs font-semibold text-muted-foreground hover:text-foreground"><MessageCircleQuestion className="size-3.5" />{t.dispute}</Link> : null}</div></div></div><p data-private-amount className={`shrink-0 text-lg font-semibold ${entry.direction === "IN" ? "text-emerald-600 dark:text-emerald-400" : entry.direction === "OUT" ? "text-red-600 dark:text-red-400" : "text-foreground"}`}>{sign}{money(entry.amount, entry.currency, locale)}</p></div>;
        })}
      </div>
