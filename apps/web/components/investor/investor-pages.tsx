...
const INVESTOR_STRINGS = {
  en: {
    ...
    reinvest: {
      ...
      calculatorTitle: "Compare the options", calculatorDesc: "Use the existing calculator to compare a payout with reinvestment before changing your preference.", calculatorAction: "Open calculator"
    },
    deposit: {
  ...
  ru: {
    ...
    reinvest: {
      ...
      calculatorTitle: "Сравните варианты", calculatorDesc: "Используйте существующий калькулятор, чтобы сравнить выплату и реинвестирование до изменения настройки.", calculatorAction: "Открыть калькулятор"
    },
    deposit: {
...
const LOCALIZED_INVESTOR_STRINGS = {
  ...
  es: {
    ...
    reinvest: { approachTitle: "Cómo funciona", approachDesc: "La reinversión puede destinar resultados disponibles a futuras operaciones tras la revisión del gestor.", whatChanges: "Qué cambia", whatChangesVal: "Los fondos elegibles pueden usarse en futuras operaciones en lugar de retirarse.", whatNotChanges: "Qué no cambia", whatNotChangesVal: "No garantiza disponibilidad, plazos ni resultados.", reviewModel: "Confirmación", reviewModelVal: "El gestor confirma el cambio antes de aplicarlo.", calculatorTitle: "Compare las opciones", calculatorDesc: "Use la calculadora existente para comparar el retiro y la reinversión antes de cambiar su preferencia.", calculatorAction: "Abrir calculadora" },
  ...
  de: {
    ...
    reinvest: { approachTitle: "So funktioniert es", approachDesc: "Verfügbare Ergebnisse können nach Prüfung künftigen Geschäften zugewiesen werden.", whatChanges: "Was sich ändert", whatChangesVal: "Geeignete Mittel können statt einer Auszahlung in künftige Geschäfte fließen.", whatNotChanges: "Was gleich bleibt", whatNotChangesVal: "Verfügbarkeit, Zeitplan und Ergebnis sind nicht garantiert.", reviewModel: "Bestätigung", reviewModelVal: "Der Manager bestätigt die Änderung vor der Anwendung.", calculatorTitle: "Optionen vergleichen", calculatorDesc: "Vergleichen Sie mit dem bestehenden Rechner Auszahlung und Reinvestition, bevor Sie Ihre Auswahl ändern.", calculatorAction: "Rechner öffnen" },
  ...
  zh: {
    ...
    reinvest: { approachTitle: "运作方式", approachDesc: "经经理确认后，可将可用结果用于未来项目。", whatChanges: "会改变什么", whatChangesVal: "符合条件的资金可用于未来项目，而不是提现。", whatNotChanges: "不会改变什么", whatNotChangesVal: "不保证项目供应、时间或结果。", reviewModel: "确认", reviewModelVal: "经理会在执行前确认变更。", calculatorTitle: "比较方案", calculatorDesc: "更改设置前，可使用现有计算器比较提现与再投资方案。", calculatorAction: "打开计算器" },
    ...
    <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
      ...
      <Card className="rounded-[1.35rem] bg-card dark:bg-graphite-900/[0.72]">
        ...
        <CardContent className="grid gap-4">
          ...
          <div className="rounded-2xl border border-border bg-muted/20 p-4 dark:border-white/10 dark:bg-white/[0.025]">
            <p className="font-semibold text-foreground">{t.reinvest.calculatorTitle}</p>
            <p className="mt-1 text-sm leading-6 text-muted-foreground">{t.reinvest.calculatorDesc}</p>
            <Link href={`/${locale}/calculator`} className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-full border border-border px-4 text-sm font-semibold text-foreground transition-colors hover:border-gold-200/40 hover:bg-muted/50 dark:border-white/10">
              <BarChart3 className="size-4 text-amber-700 dark:text-gold-100" />
              {t.reinvest.calculatorAction}
            </Link>
          </div>
        </CardContent>
      </Card>
