...
const COPY = {
  en: { eyebrow: "Help", title: "My requests", description: "Send a question to the OTIZ team and track its status here.", formTitle: "New request", formDesc: "Describe one issue per request. Include the relevant deal, report or payment when possible.", category: "Topic", categories: ["Money", "Documents", "Reports", "Account", "Other"], subject: "Short subject", message: "What happened, and what help do you need?", submit: "Send request", sent: "Your request was sent. The team can now review it.", error: "Complete the topic, subject and message fields.", history: "Request history", empty: "You have not sent any support requests yet.", telegram: "Open Telegram", reply: "Team reply", requestTypes: { PROFILE_CHANGE_REQUEST: "Change personal details", ACCOUNT_PAUSE_REQUEST: "Pause account", ACCOUNT_CLOSE_REQUEST: "Close account" }, status: { OPEN: "Received", ACKNOWLEDGED: "In review", RESOLVED: "Resolved" } },
  ru: { eyebrow: "Помощь", title: "Мои обращения", description: "Отправьте вопрос команде OTIZ и следите за его статусом здесь.", formTitle: "Новое обращение", formDesc: "Описывайте один вопрос в одном обращении. По возможности укажите сделку, отчёт или платёж.", category: "Тема", categories: ["Деньги", "Документы", "Отчёты", "Аккаунт", "Другое"], subject: "Кратко опишите вопрос", message: "Что произошло и какая помощь нужна?", submit: "Отправить обращение", sent: "Обращение отправлено. Команда может начать его проверку.", error: "Заполните тему, заголовок и описание обращения.", history: "История обращений", empty: "У вас пока нет обращений в поддержку.", telegram: "Открыть Telegram", reply: "Ответ команды", requestTypes: { PROFILE_CHANGE_REQUEST: "Изменение персональных данных", ACCOUNT_PAUSE_REQUEST: "Приостановка аккаунта", ACCOUNT_CLOSE_REQUEST: "Закрытие аккаунта" }, status: { OPEN: "Получено", ACKNOWLEDGED: "На рассмотрении", RESOLVED: "Решено" } },
  es: { eyebrow: "Ayuda", title: "Mis solicitudes", description: "Envíe una consulta al equipo de OTIZ y siga su estado aquí.", formTitle: "Nueva solicitud", formDesc: "Describa un asunto por solicitud. Indique la operación, el informe o el pago relacionado cuando sea posible.", category: "Tema", categories: ["Dinero", "Documentos", "Informes", "Cuenta", "Otro"], subject: "Asunto breve", message: "¿Qué ocurrió y qué ayuda necesita?", submit: "Enviar solicitud", sent: "Su solicitud fue enviada. El equipo ya puede revisarla.", error: "Complete el tema, el asunto y el mensaje.", history: "Historial de solicitudes", empty: "Aún no ha enviado solicitudes de soporte.", telegram: "Abrir Telegram", reply: "Respuesta del equipo", requestTypes: { PROFILE_CHANGE_REQUEST: "Cambio de datos personales", ACCOUNT_PAUSE_REQUEST: "Pausa de la cuenta", ACCOUNT_CLOSE_REQUEST: "Cierre de la cuenta" }, status: { OPEN: "Recibida", ACKNOWLEDGED: "En revisión", RESOLVED: "Resuelta" } },
  de: { eyebrow: "Hilfe", title: "Meine Anfragen", description: "Senden Sie dem OTIZ-Team eine Frage und verfolgen Sie hier den Status.", formTitle: "Neue Anfrage", formDesc: "Beschreiben Sie pro Anfrage ein Thema. Nennen Sie möglichst das zugehörige Geschäft, den Bericht oder die Zahlung.", category: "Thema", categories: ["Geld", "Dokumente", "Berichte", "Konto", "Sonstiges"], subject: "Kurzer Betreff", message: "Was ist passiert und welche Hilfe benötigen Sie?", submit: "Anfrage senden", sent: "Ihre Anfrage wurde gesendet und kann nun geprüft werden.", error: "Füllen Sie Thema, Betreff und Nachricht aus.", history: "Anfrageverlauf", empty: "Sie haben noch keine Support-Anfrage gesendet.", telegram: "Telegram öffnen", reply: "Antwort des Teams", requestTypes: { PROFILE_CHANGE_REQUEST: "Persönliche Daten ändern", ACCOUNT_PAUSE_REQUEST: "Konto pausieren", ACCOUNT_CLOSE_REQUEST: "Konto schließen" }, status: { OPEN: "Eingegangen", ACKNOWLEDGED: "In Prüfung", RESOLVED: "Erledigt" } },
  zh: { eyebrow: "帮助", title: "我的请求", description: "向 OTIZ 团队提交问题，并在此查看处理状态。", formTitle: "新请求", formDesc: "每个请求只描述一个问题，并尽可能注明相关项目、报告或付款。", category: "主题", categories: ["资金", "文件", "报告", "账户", "其他"], subject: "简短标题", message: "发生了什么，您需要什么帮助？", submit: "提交请求", sent: "请求已提交，团队现在可以开始处理。", error: "请填写主题、标题和问题描述。", history: "请求记录", empty: "您还没有提交支持请求。", telegram: "打开 Telegram", reply: "团队回复", requestTypes: { PROFILE_CHANGE_REQUEST: "更改个人资料", ACCOUNT_PAUSE_REQUEST: "暂停账户", ACCOUNT_CLOSE_REQUEST: "关闭账户" }, status: { OPEN: "已收到", ACKNOWLEDGED: "处理中", RESOLVED: "已解决" } }
} as const;
...
export default async function InvestorSupportPage(props: { params: Promise<{ locale: Locale }>; searchParams: Promise<{ sent?: string; error?: string; context?: string }> }) {
  ...
  const [requests, settings] = await Promise.all([
    prisma.operationalIncident.findMany({ where: { investorId: investor.id, incidentType: { in: ["SUPPORT_REQUEST", "PROFILE_CHANGE_REQUEST", "ACCOUNT_PAUSE_REQUEST", "ACCOUNT_CLOSE_REQUEST"] } }, orderBy: { createdAt: "desc" }, take: 30 }),
    getSiteSettings()
  ]);
  ...
  return (
    <InvestorShell locale={locale} investor={investor} active="support" eyebrow={t.eyebrow} title={t.title} description={t.description}>
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,0.8fr)]">
        <Card id="new-request" className="scroll-mt-6 rounded-[1.35rem]">
